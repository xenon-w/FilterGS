...
class LoG(nn.Module):
    ...
    def at_init_final_with_train_size(self, train_dataset_size):
        print(f'[{self.__class__.__name__}] minimum {self.gaussian.log_radius(self.counter.radius3d_min)}')
        radius3d_min = self.gaussian.activation.scaling_inverse_activation(self.counter.radius3d_min)[:, None].repeat(1, 3)
        # update radius3d_max
        self.counter.radius3d_max.fill_(self.gaussian.xyz_scale * 0.2)
    ...
    def update_by_output(self, output):
        # Safety check: ensure output has required keys
        if 'visibility_flag' not in output:
            print(f'[{self.__class__.__name__}] Warning: No visibility_flag in output, skipping update')
            return
        
        # Check if visibility_flag has valid data
        for i, vis_flag in enumerate(output['visibility_flag']):
            if 'index' not in vis_flag or vis_flag['index'].numel() == 0:
                print(f'[{self.__class__.__name__}] Warning: No visible points in visibility_flag[{i}], skipping update')
                return
        ...
        try:
            ...
            print(f'[{self.__class__.__name__}] Error in counter.update_by_output: {e}')
            print(f'[{self.__class__.__name__}] Output structure: {list(output.keys())}')
            if 'visibility_flag' in output:
                for i, vis_flag in enumerate(output['visibility_flag']):
                    print(f'[{self.__class__.__name__}] visibility_flag[{i}]: {vis_flag}')
            raise e
    ...
    def update_depth_stage(self, global_iteration):
        ...
        if flag_split.sum() > num_max_split:
            if sort_method == 'radii':
                ...
                flag_split = flag_split & (grad >= new_grad_thres)
            elif sort_method == 'mixed':
                flag_split = self._mixed_sort_selection(
                    flag_split, radii_max_max, grad, num_max_split, global_iteration, log_prefix)
    ...
    def _mixed_sort_selection(self, flag_split, radii_max_max, grad, num_max_split, global_iteration, log_prefix):
        cfg = self.densify_and_remove
        radius_ratio = getattr(cfg, 'sort_radius_ratio', 0.05)
        lambda_max = getattr(cfg, 'sort_lambda_max', 0.9)
        lambda_min = getattr(cfg, 'sort_lambda_min', 0.3)
        decay = getattr(cfg, 'sort_lambda_decay', 100000)
        # determine lambda schedule
        if decay <= 0:
            lam = lambda_min
        else:
            lam = lambda_min + (lambda_max - lambda_min) * math.exp(-max(global_iteration, 0) / decay)
        lam = float(min(max(lam, 0.0), 1.0))
        flag_candidate = flag_split.clone()
        num_candidates = int(flag_candidate.sum().item())
        if num_candidates == 0:
            return flag_split
        if 0 < radius_ratio < 1:
            keep_radius = max(1, int(num_candidates * radius_ratio))
            if keep_radius < num_candidates:
                radius_vals = radii_max_max[flag_candidate]
                radius_thres = torch.topk(radius_vals, keep_radius, largest=True).values[-1]
                flag_candidate = flag_candidate & (radii_max_max >= radius_thres)
        candidate_vals = int(flag_candidate.sum().item())
        if candidate_vals == 0:
            return flag_split
        radius_vals = radii_max_max[flag_candidate]
        grad_vals = grad[flag_candidate]
        radius_mean = radius_vals.mean()
        grad_mean = grad_vals.mean()
        radius_std = torch.clamp(radius_vals.std(), min=1e-6)
        grad_std = torch.clamp(grad_vals.std(), min=1e-6)
        radius_norm = torch.zeros_like(radii_max_max)
        grad_norm = torch.zeros_like(grad)
        radius_norm[flag_candidate] = (radius_vals - radius_mean) / radius_std
        grad_norm[flag_candidate] = (grad_vals - grad_mean) / grad_std
        score = radius_norm * lam + grad_norm * (1.0 - lam)
        num_selected = min(num_max_split, candidate_vals)
        if num_selected < candidate_vals:
            score_vals = score[flag_candidate]
            score_thres = torch.topk(score_vals, num_selected, largest=True).values[-1]
            new_flag = flag_candidate & (score >= score_thres)
        else:
            new_flag = flag_candidate
        print(f'{log_prefix} mixed sort λ={lam:.3f} select {new_flag.sum():8d}/{flag_split.sum():8d}')
        return new_flag
    ...
    def load_state_dict(self, state_dict, strict= True, split='demo'):
        ...
        # Helper function to register buffer in nested module
        def register_nested_buffer(module, key, val):
            if '.' in key:
                keys = key.split('.')
                parent_module = get_module_by_str(module, '.'.join(keys[:-1]))
                if parent_module is not None:
                    parent_module.register_buffer(keys[-1], val)
                    return True
                else:
                    print(f'[{self.__class__.__name__}] Cannot find parent module for {key}, skipping.')
                    return False
            else:
                module.register_buffer(key, val)
                return True
        
        # directly replace the tensor
        for key, val in state_dict.items():
            if split != 'train' and 'optimizer' in key:
                # Skip optimizer states silently when not in training mode
                continue
            param = get_module_by_str(self, key)
            if param is None:
                register_nested_buffer(self, key, val)
                continue
