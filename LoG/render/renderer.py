...
class NaiveRendererAndLoss(BaseRender):
    ...
    def render(self, camera, rasterizer, model, extra_params={}):
        ...
        # Print number of Gaussians entering the rasterizer
        # num_gaussians = xyz.shape[0]
        # try:
        #     from tqdm import tqdm as tqdm_module
        #     tqdm_module.write(f'[Render] Gaussians entering rasterizer: {num_gaussians}')
        # except:
        #     print(f'[Render] Gaussians entering rasterizer: {num_gaussians}', flush=True)
        ...
        if self.split == 'train' or self.split == 'val':
            ...
            if self.split == 'train':
                if 'mask_ignore' in batch.keys():
                    ...
                    if torch.is_tensor(mask_ignore):
                        mask_ignore_tensor = mask_ignore
                    else:
                        mask_ignore_tensor = torch.as_tensor(mask_ignore)
                    if mask_ignore_tensor.dim() == 2:
                        mask_ignore_tensor = mask_ignore_tensor.unsqueeze(0)
                    if mask_ignore_tensor.dim() == 4 and mask_ignore_tensor.shape[1] == 1:
                        mask_ignore_tensor = mask_ignore_tensor.squeeze(1)
                    mask_ignore_tensor = mask_ignore_tensor.to(gt_image.device, gt_image.dtype)

                    if mask_ignore_tensor.dim() != 3:
                        mask_ignore_tensor = mask_ignore_tensor.view(gt_image.shape[0], gt_image.shape[2], gt_image.shape[3])

                    mask_ignore_expanded = mask_ignore_tensor.unsqueeze(1)

                    background_value = None
                    if 'background' in output:
                        bg_entry = output['background']
                        if isinstance(bg_entry, (list, tuple)):
                            if bg_entry:
                                bg_candidate = bg_entry[0]
                                if isinstance(bg_candidate, torch.Tensor):
                                    background_value = bg_candidate.to(gt_image.device, gt_image.dtype)
                                else:
                                    background_value = torch.as_tensor(bg_candidate, device=gt_image.device, dtype=gt_image.dtype)
                        elif torch.is_tensor(bg_entry):
                            if bg_entry.dim() == 0:
                                background_value = bg_entry.to(gt_image.device, gt_image.dtype)
                            else:
                                background_value = bg_entry[0].to(gt_image.device, gt_image.dtype)
                        else:
                            background_value = torch.as_tensor(bg_entry, device=gt_image.device, dtype=gt_image.dtype)
                    if background_value is None:
                        background_value = self.background.to(gt_image.device, gt_image.dtype)

                    if background_value.dim() > 1:
                        background_value = background_value.view(-1)
                    gt_image = gt_image * mask_ignore_expanded + (1 - mask_ignore_expanded) * background_value[None, :, None, None]
                    self.calculate_loss(gt_image, render, output, mask_ignore=mask_ignore_tensor)
                    output['mask_ignore'] = mask_ignore_tensor
                else:
