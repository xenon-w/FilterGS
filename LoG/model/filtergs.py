...
from ..cuda.prepare_preprocess_inputs import prepare_preprocess_inputs
...
class FilterGS(nn.Module):
    """
    FilterGS 类：完全复现 LoG-pro val 模式的 GPU 并行筛选 + 批量计算流程
    包含 prepare、get_all、render 三个阶段
    """
    ...
    def prepare(self, rasterizer, camera, tree=None, tree_node_index_gpu=None, ancestor_path_gpu=None,
        ...
        """
        第一阶段：GPU 并行筛选可见高斯点（与 LoG-pro prepare 完全一致）
        """
        ...
        # Use caller-provided GPU buffers when available to avoid per-frame copy cost.
        ...
        # 使用 CUDA 加速的复杂筛选（视锥裁剪 + LOD 筛选 + 大半径过滤）
        ...
        try:
            ...
            vis_fallback = True
            print(f"[FilterGS] CUDA visibility computation failed: {exc}, fallback to basic")
            # 回退到基础实现
            valid_flag, depth, _ = self._visible_flag_by_camera_cuda(self.xyz, camera, padding=padding)
            all_mask = valid_flag
            all_radius2d = torch.zeros_like(self.xyz[:, 0])
            all_depth = depth
        t_vis_ms = _time_end(stage_vis_start)

        # Pack visibility data（与 LoG-pro 完全一致）
    ...
    def get_all(self, camera):
        """
        第二阶段：批量收集筛选后的属性（与 LoG-pro get_all 完全一致）
        """
    ...
    def render(self, camera, rasterizer, background=None, enable_stats=True):
        """
        第三阶段：预处理 + 渲染（与 LoG-pro render 完全一致）
        """
        if self.visibility_flag is None:
    ...
    def _compute_visibility_radius_cuda(self, rasterizer, camera, tree_node_index_gpu, ancestor_path_gpu,
        ...
        # 调用完整的 CUDA kernel（需要 compute_visibility_radius 导出存在）
