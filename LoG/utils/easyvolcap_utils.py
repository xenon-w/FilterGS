...
from easyvolcap.utils.console_utils import log, red, yellow
...
class Viewer(VolumetricVideoViewer):
    def __init__(self,
        ...
        self.runner = dotdict(
            ...
            timer_record_to_file=False,
            timer_sync_cuda=False,
            test_using_inference_mode=False,
            test_use_amp=False,
            test_amp_cached=False,
            maybe_jit_model=lambda *args, **kwargs: None,
        )
        self.use_vsync = True
    ...
    def frame(self):
        ...
        self.dynamic = dotdict()
        debug_frame = not getattr(self, '_debug_frame_logged', False)
        if debug_frame:
            log(yellow('[ViewerDebug] frame start'))

        # Render GS
        if self.render_network:
            batch = easyvolcap_camera_to_fastnb_camera(self.camera.to_batch())
            if debug_frame:
                log(yellow(f'[ViewerDebug] camera batch prepared: H={batch.camera.image_height}, W={batch.camera.image_width}'))
            with torch.no_grad():
                output = self.renderer.vis(dotdict(camera=batch), self.model)
            if debug_frame:
                keys = ','.join(output.keys())
                log(yellow(f'[ViewerDebug] renderer.vis done, keys={keys}'))
            ...
            if debug_frame:
                log(yellow(f'[ViewerDebug] image prepared: shape={tuple(image.shape)}'))

            self.quad.copy_to_texture(image)
            self.quad.draw()  # draw is typically faster by 0.5ms
            if debug_frame:
                log(yellow('[ViewerDebug] quad upload/draw done'))
        ...
        self.show_imgui()
        if debug_frame:
            log(yellow('[ViewerDebug] imgui draw done'))
